...
If you use maven to build your projects, you can put an effective pom
into the /META-INF/effective-pom.xml file of your EAR, with properties
that signal to ece-install what files to download, by specifying the
maven coordinates of the artifact.

Include these coordinates as properties that start with *escenic.* and
end with *.distribution* to get ece-install to download them from a
maven repository.

#+BEGIN_SRC nxml
<properties>
  <escenic.engine.distribution>com.escenic:engine-dist:zip:bin:${engine-version}</escenic.engine.distribution>
  <escenic.someplugin.distribution>com.escenic.plugins.someplugin:someplugin-dist:zip:${someplugin-version}</escenic.someplugin.distribution>
</properties>
#+END_SRC

In your ece-install.conf file, specify *fai_maven_repositories* as a
whitespace separated list of repositories that may hold these binaries,
and ece-install will try each repository in order to download the arifacts.
