#+TITLE: Welcome to the /usr/sbin/ece-install Guide
...
| fai_monitoring_nagios_flavour       | icinga                                            | Flavour of Nagios, possible options: icigina & nagios                                                                                 |
...
| fai_publication_domain_mapping_list | ""                                                | Mapping between publication names and their corresponding domains on the form: "one#one.com other#other.com"                          |
...
** Uninstall (most of) the packages installed by ece-install
#+BEGIN_SRC sh
...
** Remove the directories created by ece-install
#+BEGIN_SRC sh
# rm -rf \
    /var/{lib,cache,crash,log,run}/escenic \
    /{etc,opt}/escenic \
    /opt/*tomcat*
#+END_SRC


** Remove the GPG keys added by ece-install
#+BEGIN_SRC sh
...
* Exceptions
** Easily 500
{~} what now... while true; do curl -s  -I
"http://newindianexpress.com//cities/chennai/article555367.ece%C2%^C";
done


** 2012-07-02 16:24
2012-07-02 17:30:15,589 ERROR [http-8080-34]
(com.escenic.framework.search.solr.SolrSearchEngine) solr search
failed
...
request: http://localhost:8081/solr/select?sort=score
desc&fl=*&start=0&q=(smoking OR scenes:censor OR board OR damages OR
people`s OR health)+section:(3)+contenttype:
(news)+publication:(nie)+(state:(published)+expiredate:[2012-07-02T12:00:15Z
TO *]+activatedate:[* TO
2012-07-02T12:00:15Z])+publishdate:[1970-01-01T00:00:00Z TO
2012-07-02T12:00:15Z]&rows=15&wt=javabin&version=1
        at
        ...
        ... 108 more
2012-07-02 17:30:15,593 ERROR [http-8080-34]
(com.escenic.framework.search.solr.action.SearchAction) Error while
executing search
java.lang.NullPointerException
        at
        com.escenic.framework.search.solr.action.SearchAction.execute(SearchAction.java:56)
