 ...
 * One =fai_<profile>_install=1= setting to choose the profile you want to
   install or =fai_restore_from_backup=1= to restore a previously
   installed profile.
 * Additional settings related to the profile installation
   or restore operation you have selected. There are defaults for most
   of these additional parameters, so in many cases you do not need to
...
A *profile* is a pre-defined set of software components that enables a
computer to perform a particular function. The profiles that
=ece-install= can currently install are described in the following sections.


** Common Components
=ece-install= always installs a Munin node on the host, irrespective
of which profile you choose. It also installs the
Escenic-specific Munin plugins if the profile you choose includes any
Escenic server components.

The following components are included in the Editorial Server,
Presentation Server, Search Server and All In One profiles:

 * Sun Java 6 :: This is required to run Tomcat and all the Escenic
                 components.
 * Apache Tomcat application server :: This is installed and
      configured to provide:
   - Compression of textual content (previously, this was typically
     done using Apache's =mod_deflate= module)
   - Pooling, tweaked for high read/write performance
   - Proper logging, with Solr messages directed to a separate log
   - Routing information in the cookies
   - Application server access log
 * Basic Escenic configuration :: Basic Escenic configuration layers
      are installed, including RMI, database, search and application server/URIs. 
 * APR :: native library for optimal Tomcat I/O performance
 * Escenic assembly environment :: This is installed on every host in
      order to make the installation process as smooth as
      possible. You can remove it after installation if you wish.
 * Database driver :: ??
...
To install this profile in interactive mode, select the
#+BEGIN_SRC text
   1 - All in one, full stack on one host, suitable for dev & test environments
#+END_SRC
option.

To install it in FAI mode, add =fai_all_install=1= to your
=ece-install.conf= file.

** Editorial Server
This profile contains all the components that need to be installed on an
Escenic *editorial server*. An editorial server (sometimes also called
a publication server) hosts a Content Engine used for editorial
...
To install it in FAI mode, add =fai_editor_install=1= to your
=ece-install.conf= file.
...
| ~fai_db_sql_only~                     | 0                                                            | Only run the SQL, but don't install the DB software. Useful for setting up remote DBs, like Amazon RDS   |
