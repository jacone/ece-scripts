...
- *keep\_off\_wget\_user\_agent* Whenever using *wget* to download
  software, ece-install will add something to the UA string of *wget*
  so that it's possible to identify what the command downloads. If
  you wish to turn this feature off, set this to *0*.
