...
/tmp/nursery-skeleton-and-security.tar.gz should now have everything
...
| fai\_publication\_war            | "WF or ECE demo WAR" | WAR to to base the new publication on               |
...
The root directory of all Escenic software binaries, /opt/escenic,
together with the root directory for the configuration files may be
overwritten in ece-install.conf by setting these variables:
