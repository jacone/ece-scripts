...
If your systmem uses sudo to provide access to the root user, we
recommend becoming root before running ece-install. To become root on
a system using sudo, do:
#+BEGIN_SRC sh
$ sudo su -
#+END_SRC
