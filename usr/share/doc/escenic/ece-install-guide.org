...
This will install a Munin gatherer and make its interface available 
...
/var/backups/escenic
#+END_SRC
and the ece-install script will hence expect to find them here (when
running interactively), asking you from which one you wish to restore
the system:
...
*** Running in FAI mode
If you're running in FAI mode, you specify the file to restore from
with:
#+BEGIN_SRC sh
