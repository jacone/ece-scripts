...
An Import Configuration Archive (ICA) is a collection of files packaged in an archive that can import data into Escenic Engine. It can be deployed to any machine that has Escenic Engine running on it that was installed by ece-install.
...
All ICA's delivered to the VOSA operators are =zip=
archives with a strict structure, as defined in [[Specifying an ICA]]

** Specifying an ICA
When creating a new ICA for your project, the following directory
...
When an ICA is gotten from a developer, the VOSA operator
