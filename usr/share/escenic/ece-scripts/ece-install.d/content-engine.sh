...
function leave_content_engine_trails() {
  if [ ${fai_editor_install-0} -eq 1 ]; then
    ...
    leave_trail "trail_editor_shutdown=${fai_editor_shutdown-${default_app_server_shutdown}}"
  fi
  if [ ${fai_presentation_install-0} -eq 1 ]; then
    leave_trail "trail_presentation_port=${fai_presentation_port-${default_app_server_port}}"
    leave_trail "trail_presentation_shutdown=${fai_presentation_shutdown-${default_app_server_shutdown}}"
  fi
  if [ ${fai_analysis_install-0} -eq 1 ]; then
    leave_trail "trail_analysis_port=${fai_analysis_port-${default_app_server_port}}"
    leave_trail "trail_analysis_shutdown=${fai_analysis_shutdown-${default_app_server_shutdown}}"
  fi
