...
filebucket { 'main':
  server => 'puppet.velthovenhome.net',
  path   => false,
}
...
node 'issues.velthovenhome.net' {
    include issues
#    $fileNames = split( generate("/usr/bin/find", $puppetMasterTemplatePath,"-type", "f", "-print0"), "\0")
#    templated_file{$fileNames:}
}
